...
taxa_country_rlv <-
  ...
  filter(
    ...
    !is.na(scientific_name) # Exclude regional scientific names without an officially accepted match
  )
```
...
taxa_country_rlv %>%
  ...
  write_csv(here::here("data", "interim", "countries_per_species.csv"), na = "")
```

# Weighted Red list values

Calculate the mean wrl value per country:

```{r}
wrl_values_per_country <-
  wrl_data %>%
  group_by(country_code, country_name, part_of_eu, area, population) %>%
  summarize(
    mean_rlc = mean(rlc_numeric, na.rm = TRUE)
  ) %>%
  select(country_code, mean_rlc, country_name, part_of_eu, area, population)

write_csv(wrl_values_per_country, here::here("data", "processed", "wrl", "wrl_values_per_country.csv"), na = "")
```
