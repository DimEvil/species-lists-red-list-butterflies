...
The source data is maintained in a Google Spreadsheet, which is copied to `data/raw` in `src/dwc_mapping.Rmd`: we will use those files as source files:

```{r}
taxa_regional <- read_csv(here::here("data", "raw", "distribution.csv"))
regional_threat_scores <- read_csv(here::here("data", "raw", "rlc.csv"))
country_info <- read_csv(here::here("data", "raw", "regions.csv"))
specieslist <- read_csv(here::here("data", "raw", "taxa.csv"))
```
