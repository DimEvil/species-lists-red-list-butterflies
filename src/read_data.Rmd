...
This document describes how we import the "National checklists and red lists for prioritising European butterfly conservations actions" dataset from the basic [Googlesheets document](https://docs.google.com/spreadsheets/d/1RvxpOYf2ZrTu9nsTLumoi-G-GGhh6_lV37TNtPiVES4/edit?ts=5b1f937f#gid=0) as [csv files](https://github.com/inbo/red-lists-european-butterflies-checklist/tree/master/data/interim) on GitHub. It further describes how we match the species information to the GBIF backbone.
...
The original spreadsheet can be found [here](https://docs.google.com/spreadsheets/d/1RvxpOYf2ZrTu9nsTLumoi-G-GGhh6_lV37TNtPiVES4/edit?ts=5b1f937f#gid=1323544344). We need to retrieve this spreadsheet and select the specific worksheet first:
...
This spreadsheet contains four different worksheets: `taxa_europe`, `taxa_regional`, `regional_threat_scores` and `references`. We select these and save them as separate data frames.
...
We retrieve the taxonomic information for each species in `taxa_europe`. We do this using the gbif_species_name_match() function provided by the `inborutils` package: 

```{r}
taxa_europe %<>% gbif_species_name_match(name_col = "SpeciesnameEurope", 
                                        gbif_terms = c('usageKey', 'scientificName', 'rank', 'matchType', 
                                                       'kingdom', 'phylum', 'class', 'order', 'family', 
                                                       'genus', 'confidence', 'synonym', 'status'))
```

We do the same for `taxa_regional`:

```{r}
taxa_regional %<>% gbif_species_name_match(name_col = "SpeciesnameLocal", 
                                        gbif_terms = c('usageKey', 'scientificName', 'rank', 'matchType', 
                                                       'kingdom', 'phylum', 'class', 'order', 'family', 
                                                       'genus', 'confidence', 'synonym', 'status'))
```
