...
This spreadsheet contains three relevant different worksheets: `taxa_regional`, `regional_threat_scores` and `references`. We select these here and save them as separate data frames. We will then continue with the `taxa_regional` as this is the original dataframe.
...
Select columns for which `raw_use = Y`:
...
```{r}
taxon %<>% distinct(raw_taxon_id, .keep_all = TRUE)
```
...
```{r}
taxon %<>% mutate(identificationReferences = raw_reference_species_list) 
```
...
```{r}
distribution %<>% mutate(establishmentMeans = raw_status)
