...
```{r}
distribution %>%
  group_by(input_reference_red_list) %>%
  count()
```
...
```{r}
distribution %<>% mutate(source =
  str_replace(input_reference_red_list, ";", " | ")
)
```
...
```{r}
distribution %>%
  filter(input_reference_red_list != source) %>%
  group_by(input_reference_red_list, source) %>%
  count()
```
