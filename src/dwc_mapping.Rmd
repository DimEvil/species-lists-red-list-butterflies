---
title: "Darwin Core mapping"
subtitle: "Red List Butterflies Europe EU_list"
author:
- Lien Reyserhove
- Dimitri Brosens
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
    toc_depth: 3
    toc_float: true
    number_sections: true
---

This document describes how we map the checklist data to Darwin Core. 

# Setup

```{r setup, echo = FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)
```

Set locale (so we use UTF-8 character encoding):

```{r}
# This works on Mac OS X, might not work on other OS
Sys.setlocale("LC_CTYPE", "en_US.UTF-8")
```

Load libraries:

```{r}

# None core tidyverse packages:
library(magrittr)  # For %<>% pipes

# Other packages
library(tidyverse) # For data transformations
library(janitor)    # For cleaning input data
library(knitr)      # For nicer (kable) tables
library(readxl)     # To read excel files
library(stringr)    # To perform string operations
library(digest)     # To generate hashes
library(rgbif)      # To Match GBIF
library(assertable) # because it sneeded for rGBIF
library(inborutils) # wrap GBIF api data
library(dplyr)

```

Set file paths (all paths should be relative to this script):

```{r}
# Raw files:
raw_data_file = "../data/interim/taxa_regional.csv"

# Processed files:
dwc_taxon_fileEU = "../data/processed/dwc_checklistEU/taxon.csv"
dwc_distribution_file = "../data/processed/dwc_checklistEU/distribution.csv"

```

# Read and pre-process raw data

Create a data frame `raw_data` from the source data:

# Pre-processing



# Read the source data:
```{r}
raw_data <- read_csv("../data/interim/taxa_regional.csv")
references <- read_csv ("../data/interim/references.csv")
#View(raw_data)
#View(references)
```

# Join references

```{r}
raw_data <- taxa_regional %>% left_join(references, by = c("CountryCode" = "CountryCode"))
```

```{r}
raw_data %<>%
  remove_empty("rows") %>%    # Remove empty rows
  clean_names()               # Have sensible (lowercase) column names
```




## Further pre-processing:

Add prefix `raw_` to all column names to avoid name clashes with Darwin Core terms:

```{r}
colnames(raw_data) <- paste0("raw_", colnames(raw_data))
```
 
Preview data:

```{r}
head(raw_data, n = 100)
```
#Create taxonID

```{r}
# Vectorize the digest function (The digest() function isn't vectorized. So if you pass in a vector, you get one value for the whole vector rather than a digest for each element of the vector):
vdigest <- Vectorize(digest)

raw_data %<>% mutate(taxonID = paste("red-lists-european-butterflies-checklist", "taxon", vdigest (raw_speciesname_europe, algo="md5"), sep=":"))
```

Preview data:

```{r}
head(raw_data , n = 100)


```

## Create taxon core

```{r}
taxon <- raw_data
# View(taxon)
```

### Term mapping

Map the data to [Darwin Core Taxon](http://rs.gbif.org/core/dwc_taxon_2015-04-24.xml).
 
### Use unique values in taxon

```{r}
taxon %<>% 
  select(raw_speciesname_local, raw_speciesname_europe,taxonID, raw_use, raw_reference_species_list, raw_reference_red_list, raw_country) %>%
  filter(raw_use == 'y') %>%
  distinct(taxonID, .keep_all = TRUE) 
  head(taxon)
```



### language

```{r}
taxon %<>% mutate(language = "en")
```


### license

```{r}
taxon %<>% mutate(license = "http://creativecommons.org/publicdomain/zero/1.0/")

```

### rightsHolder

```{r}
taxon %<>% mutate(rightsHolder = "INBO")  
```


### datasetID

```{r}
taxon  %<>% mutate(datasetID = "to_be_completed")
```


### datasetName

```{r}
taxon %<>% mutate(datasetName = "red-lists-european-butterflies-checklist")
```

### nomenclaturalCode

```{r}
taxon %<>% mutate(nomenclaturalCode = "ICZN")
```

## systematics

```{r}
taxon %<>% mutate(kingdom = "Animalia") %>%
           mutate(class = "Insecta") %>%
           mutate (order= "Lepidoptera")
head(taxon)
```
## renaming



```{r}
taxon %<>%
      rename(scientificName = raw_speciesname_europe) %>%
      rename(references = raw_reference_species_list)

```

### look at data

```{r}
head(taxon)

```


 
## Post-processing

Remove the original columns:

```{r eval=FALSE, include=FALSE}
taxon %<>% select(-starts_with("raw_"))
```

Sort on `taxonID`:

```{r}
taxon %<>% arrange(taxonID)
```

Preview data:

```{r}
head(taxon)
```

Save to CSV:

```{r}
write.csv(taxon, file = dwc_taxon_fileEU, na = "", row.names = FALSE, fileEncoding = "UTF-8")
```

# Create distribution extension

```{r start_distribution}
distribution <- raw_data
head(distribution)
```

## Term mapping

Map the data to [Species Distribution](http://rs.gbif.org/extension/gbif/1.0/distribution.xml).

### Use unique values in taxon

```{r}
distribution %<>% 
  select(raw_speciesname_europe,taxonID, raw_use, raw_country_code, raw_red_list_category, raw_comments, raw_status, raw_reference_red_list) %>%
  filter(raw_use == 'y') 

head(distribution)
```



### taxonID

### scientificName

```{r}
distribution %<>%
    rename(scientificName = raw_speciesname_europe) 
```


### locationID

### locality


### countryCode


```{r}
distribution %<>%
      rename(countryCode = raw_country_code)

```

### establishmentMeans


```{r}
distribution %<>%
      rename(establishmentMeans = raw_status)

```
## renaming



```{r}
distribution %<>%
      rename(source = raw_reference_red_list)

```

### occurrenceStatus


### treadStatus


```{r}
distribution %<>%
      rename(treadStatus = raw_red_list_category)

```


### eventDate
### source

## Post-processing

Remove the original columns:

```{r}
distribution %<>% select(-starts_with("raw_"))
```

Sort on `taxonID`:

```{r}
distribution %<>% arrange(taxonID)
```

Preview data:

```{r}
distribution %>%
 # mutate(source = substr(source, 1, 10)) %>% # Shorten source to make it easier to display
  head()
```

Save to CSV:

```{r}
write.csv(distribution, file = dwc_distribution_file, na = "", row.names = FALSE, fileEncoding = "UTF-8")

```

