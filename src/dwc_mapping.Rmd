...
Some names on the regional checklists however, have names (`scientific_name_regional`) where that could not be matched (e.g. as synonyms) with names (`scientific_name`) on the official checklist:

```{r}
input_data %>%
  filter(is.na(family)) %>% # Assesses if there was a match with input_taxa and thus better than is.na(scientific_name)
  select(scientific_name_regional, scientific_name, region_code, comments) %>%
  arrange(scientific_name_regional)
```

We want to restrict the list to names that are included on `input_taxa`:
...
```{r}
taxon %<>% mutate(dwc_datasetName = "National checklists and red lists for prioritising European butterfly conservation actions")
```
...
distribution %<>% mutate(dwc_locality = case_when(
  ...
  # Use e.g. "Corvo, Azores" for island groups
  str_detect(region_code, "MA_AZ_") ~ paste(region_name, "Azores", sep = ", "),
  str_detect(region_code, "MA_MA_") ~ paste(region_name, "Madeira", sep = ", "),
  str_detect(region_code, "MA_CA_") ~ paste(region_name, "Canary Islands", sep = ", "),
